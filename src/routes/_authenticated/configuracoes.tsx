...
import {
  ...
  useSalvarSettings,
  useIntegrations,
  useDesconectarIntegration,
  useSalvarIntegration,
  settingsKeys,
} from "@/features/configuracoes/hooks/use-settings";
import {
  DEFAULT_AGENDA_SETTINGS,
  DEFAULT_FINANCEIRO_SETTINGS,
  DEFAULT_IA_SETTINGS,
  DEFAULT_NOTIFICACOES_SETTINGS,
  INTEGRATION_DESCRIPTIONS,
  INTEGRATION_LABELS,
  INTEGRATION_STATUS_CLASSES,
...
export const Route = createFileRoute("/_authenticated/configuracoes")({
  head: () => ({
    meta: [{ title: "Configurações — Corretor CRM" }, { name: "robots", content: "noindex" }],
  }),
    ...
    <div className="flex items-center gap-4 py-3 border-b last:border-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={cn(
              ...
              INTEGRATION_STATUS_CLASSES[status],
            )}
          >
      ...
      <div className="shrink-0">
        {isConnected ? (
          ...
          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={onConectar}>
            Conectar <ChevronRight className="ml-1 h-3 w-3" />
          </Button>
        ) : (
...
function AgendaConfig() {
  ...
  useEffect(() => {
    setLocal({ ...DEFAULT_AGENDA_SETTINGS, ...(raw as Partial<AgendaSettings>) });
  }, [raw]);
  const DIAS = [
    { v: 1, l: "Seg" },
    { v: 2, l: "Ter" },
  ...
  const toggleDia = (dia: number) =>
    setLocal((p) => ({
      ...p,
      dias_uteis: p.dias_uteis.includes(dia)
        ? p.dias_uteis.filter((d) => d !== dia)
        : [...p.dias_uteis, dia].sort(),
    }));
  const handleSalvar = async () => {
    await salvar.mutateAsync(local as unknown as Record<string, unknown>);
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setLocal((p) => ({ ...p, horario_inicio: e.target.value }))}
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => setLocal((p) => ({ ...p, horario_fim: e.target.value }))}
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <div className="flex items-center justify-between mb-2">
            <Label className="text-xs">Duração padrão da visita</Label>
            <span className="text-sm font-medium">{local.duracao_visita} min</span>
          </div>
          <Slider
            min={15}
            max={180}
            step={15}
            value={[local.duracao_visita]}
            onValueChange={([v]) => setLocal((p) => ({ ...p, duracao_visita: v }))}
          />
        </div>
        <div>
          <div className="flex items-center justify-between mb-2">
            <Label className="text-xs">Duração padrão do follow-up</Label>
            <span className="text-sm font-medium">{local.duracao_followup} min</span>
          </div>
          <Slider
            min={5}
            max={120}
            step={5}
            value={[local.duracao_followup]}
            onValueChange={([v]) => setLocal((p) => ({ ...p, duracao_followup: v }))}
          />
        </div>
        <div>
          <div className="flex items-center justify-between mb-2">
            <Label className="text-xs">Lembrete antecipado</Label>
            <span className="text-sm font-medium">{local.lembrete_minutos} min antes</span>
          </div>
          <Slider
            min={5}
            max={120}
            step={5}
            value={[local.lembrete_minutos]}
            onValueChange={([v]) => setLocal((p) => ({ ...p, lembrete_minutos: v }))}
          />
        </div>
      </div>
      <Button onClick={handleSalvar} disabled={salvar.isPending} className="w-full">
        {salvar.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar configurações
        de agenda
      </Button>
    </div>
  );
...
function FinanceiroConfig() {
  ...
  useEffect(() => {
    setLocal({ ...DEFAULT_FINANCEIRO_SETTINGS, ...(raw as Partial<FinanceiroSettings>) });
  }, [raw]);
  const handleSalvar = async () => {
    await salvar.mutateAsync(local as unknown as Record<string, unknown>);
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div>
        <div className="flex items-center justify-between mb-2">
          <Label className="text-xs">Percentual padrão de comissão</Label>
          <span className="text-sm font-medium">{local.comissao_padrao}%</span>
        </div>
        <Slider
          min={0.5}
          max={20}
          step={0.5}
          value={[local.comissao_padrao]}
          onValueChange={([v]) => setLocal((p) => ({ ...p, comissao_padrao: v }))}
        />
        <p className="text-xs text-muted-foreground mt-1">
          Valor pré-preenchido ao criar uma nova comissão.
        </p>
      </div>
      <div>
        ...
        <Input
          ...
          value={local.meta_mensal || ""}
          onChange={(e) =>
            setLocal((p) => ({ ...p, meta_mensal: parseFloat(e.target.value) || 0 }))
          }
        />
      </div>
      <Button onClick={handleSalvar} disabled={salvar.isPending} className="w-full">
...
function IAConfig() {
  ...
  useEffect(() => {
    setLocal({ ...DEFAULT_IA_SETTINGS, ...(raw as Partial<IASettings>) });
  }, [raw]);
  const handleSalvar = async () => {
    await salvar.mutateAsync(local as unknown as Record<string, unknown>);
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div>
        ...
        <Select
          value={local.provider_padrao}
          onValueChange={(v) => setLocal((p) => ({ ...p, provider_padrao: v }))}
        >
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Configure as chaves de API na aba Integrações.
        </p>
      </div>
      <div>
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">0 = mais focado · 1 = mais criativo</p>
          </div>
        ...
        <Slider
          ...
          onValueChange={([v]) => setLocal((p) => ({ ...p, temperatura: v }))}
        />
      </div>
      <div>
        <div className="flex items-center justify-between mb-2">
          <Label className="text-xs">Máximo de tokens por resposta</Label>
          <span className="text-sm font-medium">{local.max_tokens}</span>
        </div>
        <Slider
          min={200}
          max={4000}
          ...
          onValueChange={([v]) => setLocal((p) => ({ ...p, max_tokens: v }))}
        />
      </div>
      <div className="flex items-center justify-between">
        <div>
          <Label className="text-xs">Manter histórico de respostas</Label>
          <p className="text-xs text-muted-foreground">Salva as últimas respostas no AI Hub</p>
        </div>
        <Switch
          checked={local.historico_ativo}
          onCheckedChange={(v) => setLocal((p) => ({ ...p, historico_ativo: v }))}
        />
      </div>
      <Button onClick={handleSalvar} disabled={salvar.isPending} className="w-full">
        {salvar.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar configurações
...
function NotificacoesConfig() {
  ...
  useEffect(() => {
    setLocal({ ...DEFAULT_NOTIFICACOES_SETTINGS, ...(raw as Partial<NotificacoesSettings>) });
  }, [raw]);
  const itens = [
    {
      key: "followup_lembrete",
      label: "Lembrete de follow-ups",
      desc: "Notifica antes do horário agendado",
    },
    {
      key: "tarefa_lembrete",
      label: "Lembrete de tarefas",
      desc: "Alerta de tarefas próximas do prazo",
    },
    {
      key: "meta_alerta",
      label: "Alertas de meta",
      desc: "Quando atingir 75% e 100% da meta mensal",
    },
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div className="space-y-3">
        {itens.map(({ key, label, desc }) => (
          <div key={key} className="flex items-center justify-between gap-4 py-2">
            ...
            <Switch
              checked={local[key as keyof NotificacoesSettings] as boolean}
      ...
      <Button onClick={handleSalvar} disabled={salvar.isPending} className="w-full">
        {salvar.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar notificações
      </Button>
    </div>
  );
}
...
function IntegracoesConfig() {
  ...
  const getStatus = (p: IntegrationProvider): IntegrationStatus =>
  ...
  useEffect(() => {
    ...
    if (gcStatus === "success") {
      ...
      toast.error(
        "Erro ao conectar o Google Calendar. Verifique as configurações e tente novamente.",
      );
      window.history.replaceState({}, "", "/configuracoes");
    }
  }, [queryClient]);
  ...
  const handleAbrirWp = () => {
    ...
    const cfg = existing?.configuration as
    ...
    <div className="space-y-6 max-w-2xl">
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">
            Configure as chaves de API para ativar o AI Hub
          </CardDescription>
        </CardHeader>
        <CardContent>
          {AI_PROVIDERS.map((p) => (
          ...
          <div className="mt-3 p-3 rounded-lg bg-muted/50 text-xs text-muted-foreground">
            🔒 As chaves de API são armazenadas criptografadas e nunca expostas no frontend.
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">
            Conecte canais de mensagem para a Central de Comunicação
          </CardDescription>
        </CardHeader>
        <CardContent>
          {COMUNICACAO.map((p) => (
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">
            Sincronize com ferramentas do Google
          </CardDescription>
        </CardHeader>
        <CardContent>
          {PRODUTIVIDADE.map((p) => (
            <IntegrationCard
              ...
              status={getStatus(p)}
              onDesconectar={() => handleDesconectar(p)}
              onConectar={p === "google_calendar" ? handleConectarGoogleCalendar : undefined}
            />
          ))}
        </CardContent>
      </Card>
      ...
      <Dialog open={wpDialog} onOpenChange={setWpDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setWpForm((p) => ({ ...p, base_url: e.target.value }))}
              />
              <p className="text-xs text-muted-foreground mt-1">
                Endereço do servidor Evolution API (sem barra no final)
              </p>
            </div>
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Chave de autenticação da sua instância Evolution
              </p>
            </div>
            <div>
              ...
              <Input
                ...
                value={wpForm.instance_name}
                onChange={(e) => setWpForm((p) => ({ ...p, instance_name: e.target.value }))}
              />
              <p className="text-xs text-muted-foreground mt-1">
                Nome configurado na Evolution API
              </p>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setWpDialog(false)}>
              Cancelar
            </Button>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Obtenha em console.anthropic.com → API Keys. A chave começa com <code>sk-ant-</code>
                .
              </p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
              🔒 A chave é armazenada de forma segura no seu perfil e usada apenas nas chamadas do
              AI Hub.
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setClaudeDialog(false)}>
              Cancelar
            </Button>
    ...
    <div className="space-y-6 max-w-lg">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm">Log de acessos</p>
              <p className="text-xs text-muted-foreground">Histórico de logins e ações</p>
            </div>
            <Button size="sm" variant="outline" className="h-7 text-xs" disabled>
              Em breve
            </Button>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Baixe todos os seus dados em CSV</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm">Importar dados</p>
              <p className="text-xs text-muted-foreground">Importe clientes via CSV</p>
            </div>
            <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5" disabled>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Sobre o CRM</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          ...
          <div className="flex justify-between">
            <span>Stack</span>
            <span className="text-xs">TanStack + Supabase + React 19</span>
          </div>
          <div className="flex justify-between">
            <span>Módulos ativos</span>
            <span className="text-xs">12 de 13</span>
          </div>
        </CardContent>
      </Card>
    </div>
      ...
      <div className="flex items-center justify-between border-b px-6 py-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Personalize o CRM conforme sua necessidade
          </p>
        </div>
      </div>
        ...
        <Tabs defaultValue="perfil">
          ...
          <TabsContent value="perfil">
            <div className="space-y-4 max-w-lg">
              <p className="text-sm text-muted-foreground">
                Suas informações de perfil são gerenciadas na página de Perfil.
              </p>
