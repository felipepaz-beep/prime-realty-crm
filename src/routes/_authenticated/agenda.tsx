...
import {
  ...
  useDuplicarActivity,
  useRemoverActivity,
  useAtualizarActivity,
} from "@/features/agenda/hooks/use-activities";
...
export const Route = createFileRoute("/_authenticated/agenda")({
  head: () => ({
    meta: [{ title: "Agenda — Corretor CRM" }, { name: "robots", content: "noindex" }],
  }),
...
function AgendaPage() {
  ...
  const filtrosBase: ActivityFiltros = {
    busca: busca || undefined,
    type: tiposFiltro.length > 0 ? tiposFiltro : undefined,
    ordenarPor: "scheduled_at",
    ordem: "asc",
    porPagina: 50,
  };
  ...
  const filtrosAtivos: ActivityFiltros =
    view === "semana"
      ? {
          ...filtrosBase,
          dateStart: startOfWeek(dataSelecionada, { locale: ptBR }).toISOString(),
          dateEnd: endOfWeek(dataSelecionada, { locale: ptBR }).toISOString(),
        }
      : view === "tarefas"
        ? { ...filtrosBase, type: ["TASK"], status: ["PENDING", "IN_PROGRESS"] }
        : view === "followups"
          ? { ...filtrosBase, type: ["FOLLOWUP"], status: ["PENDING", "IN_PROGRESS"] }
          : view === "lista"
            ? filtrosBase
            : {
                ...filtrosBase,
                dateStart: startOfDay(dataSelecionada).toISOString(),
                dateEnd: endOfDay(dataSelecionada).toISOString(),
              };
  ...
  const diasSemana = Array.from({ length: 7 }, (_, i) =>
    addDays(startOfWeek(dataSelecionada, { locale: ptBR }), i),
  );
        ...
        <div className="flex-1 overflow-auto p-6">
          {isLoading ? (
            ...
            <div className="grid grid-cols-7 gap-3">
              {diasSemana.map((dia) => {
                const atividadesDia = activities.filter((a) =>
                  isSameDay(new Date(a.scheduled_at ?? a.due_at ?? ""), dia),
                );
                return (
                  <div key={dia.toISOString()} className="min-w-0">
                    <div
                      className={`text-center mb-2 ${isSameDay(dia, new Date()) ? "font-bold text-primary" : "text-muted-foreground"}`}
                    >
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {busca ? "Tente outro termo." : "Crie sua primeira atividade."}
                </p>
              </div>
      ...
      <Dialog open={novaAberta} onOpenChange={setNovaAberta}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <ActivityForm
            ...
            isLoading={criarActivity.isPending}
          />
        </DialogContent>
      </Dialog>
