...
} from "@/features/comunicacao/hooks/use-communication";
import {
  CHANNEL_COLORS,
  CHANNEL_LABELS,
  CONVERSATION_CHANNELS,
  STATUS_LABELS,
} from "@/features/comunicacao/types";
...
export const Route = createFileRoute("/_authenticated/whatsapp")({
  head: () => ({
    meta: [{ title: "Comunicação — Corretor CRM" }, { name: "robots", content: "noindex" }],
  }),
  ...
  return (
    <div className={cn("flex mb-1", isOut ? "justify-end" : "justify-start")}>
      ...
      >
        ...
        <div
          className={cn("flex items-center gap-1 mt-0.5", isOut ? "justify-end" : "justify-start")}
        >
  ...
  return (
    ...
    >
      <Avatar className="h-10 w-10 shrink-0">
        <AvatarFallback className="text-xs font-medium">{getInitials(nome)}</AvatarFallback>
      </Avatar>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1">
          <p className="text-sm font-medium truncate">{nome}</p>
          <span className="text-[10px] text-muted-foreground shrink-0">
            {formatarDataConversa(conversa.last_message_at)}
          </span>
        </div>
        <div className="flex items-center justify-between gap-1 mt-0.5">
          <p className="text-xs text-muted-foreground truncate">
            {conversa.last_message ?? "Nenhuma mensagem ainda"}
          </p>
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          <span
            className={cn(
              ...
              CHANNEL_COLORS[conversa.channel],
            )}
          >
            {CHANNEL_LABELS[conversa.channel]}
          </span>
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            ...
            {c.proximo_followup && (
              <div>
                <p className="text-muted-foreground">Próximo follow-up</p>
                <p className="font-medium">
                  {new Date(c.proximo_followup).toLocaleDateString("pt-BR")}
                </p>
              </div>
            )}
            {c.ultimo_contato && (
              <div>
                ...
                <p>{new Date(c.ultimo_contato).toLocaleDateString("pt-BR")}</p>
              </div>
            )}
          ...
          <Button asChild variant="outline" size="sm" className="w-full text-xs">
            <Link to="/clientes/$clienteId" params={{ clienteId: conversa.client_id }}>
              Ver cadastro completo <ChevronRight className="ml-1 h-3 w-3" />
            </Link>
          </Button>
        </div>
      </ScrollArea>
...
function ChatWindow({
  ...
  useEffect(() => {
    if (conversa?.unread_count && conversa.unread_count > 0) marcarLida.mutate(conversaId);
  }, [conversaId, conversa?.unread_count]);
    ...
    <div className="flex flex-1 overflow-hidden">
      <div className="flex flex-col flex-1 min-w-0">
        <div className="flex items-center gap-3 border-b px-4 py-3 shrink-0">
          <Button variant="ghost" size="icon" className="h-8 w-8 lg:hidden" onClick={onFechar}>
          ...
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => setMostrarSidebar(!mostrarSidebar)}>
                  {mostrarSidebar ? "Ocultar" : "Exibir"} informações
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem
        ...
        {conversa.status === "open" ? (
          <div className="border-t px-4 py-3 shrink-0">
            ...
            {isWhatsApp && !canalConectado && (
              <p className="text-[10px] text-amber-600 dark:text-amber-400 mt-1.5 text-center">
                ...
                <Link to="/configuracoes" search={{}} params={{}} className="underline">
                  Configurar
                </Link>
              </p>
            )}
          ...
          <div className="border-t px-4 py-3 text-center">
            <p className="text-xs text-muted-foreground">Esta conversa está encerrada.</p>
          </div>
        )}
      </div>
...
function WhatsAppPage() {
  ...
  const filtros: ConversationFiltros = {
    busca: busca || undefined,
    channel: canalFiltro === "all" ? undefined : canalFiltro,
    status: statusFiltro === "all" ? undefined : (statusFiltro as "open" | "closed"),
    porPagina: 50,
  };
      ...
      {!whatsappConectado && (
        <div className="flex items-center gap-2 border-b bg-amber-50 dark:bg-amber-900/10 px-4 py-2 shrink-0">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-400 flex-1">
            ...
            <Link to="/configuracoes" search={{}} params={{}} className="underline font-medium">
            ...
            <div className="flex flex-col h-full border-r">
              <div className="px-4 py-3 border-b shrink-0">
                ...
                <div className="flex gap-1.5 mt-2">
                  ...
                  <Select
                    value={canalFiltro}
                    onValueChange={(v) => setCanalFiltro(v as typeof canalFiltro)}
                  >
                    <SelectTrigger className="h-7 text-xs flex-1">
          ...
          <ResizablePanel defaultSize={72}>
            {conversaAtiva ? (
              <ChatWindow
                ...
                canalConectado={whatsappConectado}
                onFechar={() => setConversaAtiva(null)}
              />
            ) : (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center p-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 max-w-xs">
                    Selecione uma conversa para visualizar o histórico, ou inicie uma nova conversa
