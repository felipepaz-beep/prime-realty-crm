...
import {
  ...
  useFunilConversao,
  useOrigemLeads,
  useAtividadesAnalytics,
  useDocumentosAnalytics,
  useConversasAnalytics,
  useFinanceiroRelatorio,
} from "@/features/relatorios/hooks/use-analytics";
...
export const Route = createFileRoute("/_authenticated/relatorios")({
  head: () => ({
    meta: [{ title: "Relatórios — Corretor CRM" }, { name: "robots", content: "noindex" }],
  }),
        ...
        <div className="flex items-start justify-between">
          ...
          <div
            className={cn(
              ...
              destaque ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground",
            )}
          >
...
function RelatorioExecutivo() {
  ...
  const cards = [
    ...
    { titulo: "Leads perdidos", valor: kpis?.leads_perdidos ?? 0, icon: ArrowDown, destaque: true },
    {
      ...
      valor: kpis?.followups_atrasados ?? 0,
      icon: Calendar,
      destaque: (kpis?.followups_atrasados ?? 0) > 0,
    },
    {
      titulo: "Taxa de conversão",
      valor: MetricsService.formatarPct(kpis?.taxa_conversao_geral ?? 0),
      icon: TrendingUp,
    },
  ];
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {kpis && (
            <ExportButton
              data={[kpis] as unknown as Record<string, unknown>[]}
              filename="executivo"
            />
          )}
          <Button
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {isLoading
          ...
          : cards.map(({ titulo, valor, icon, destaque }) => (
              <KpiCard key={titulo} titulo={titulo} valor={valor} icon={icon} destaque={destaque} />
            ))}
      </div>
      {kpis && (
        <Card>
          <CardContent className="pt-5">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 text-center">
              <div>
                ...
                <p className="text-xl font-bold">{kpis.score_medio ?? "—"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Visitas agendadas</p>
                <p className="text-xl font-bold">{kpis.visitas_agendadas}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Propostas enviadas</p>
                <p className="text-xl font-bold">{kpis.propostas_enviadas}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Conversão geral</p>
                <p className="text-xl font-bold text-emerald-600">
                  {MetricsService.formatarPct(kpis.taxa_conversao_geral)}
                </p>
              </div>
            </div>
          </CardContent>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {funil && (
          <ExportButton
            data={funil as unknown as Record<string, unknown>[]}
            filename="funil_vendas"
          />
        )}
      </div>
      {isLoading ? (
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">% de participação</CardTitle>
            </CardHeader>
...
function RelatorioClientes() {
  ...
  const chartData = (origem ?? []).slice(0, 8).map((o) => ({
    ...
    total: o.total,
    convertidos: o.convertidos,
  }));
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-muted-foreground">
          Origem dos leads e taxa de conversão
        </p>
        {origem && (
          <ExportButton
            data={origem as unknown as Record<string, unknown>[]}
            filename="origem_leads"
          />
        )}
      </div>
      {isLoading ? (
              ...
              {(origem ?? []).map((o) => (
                <div key={o.origem}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="font-medium tabular-nums">
                      {o.total} leads · {o.taxa_conversao ?? 0}%
                    </span>
                  </div>
...
function RelatorioAtividades() {
  ...
  const porTipo = (atividades ?? []).reduce<Record<string, { total: number; atrasadas: number }>>(
    (acc, a) => {
      if (!acc[a.type]) acc[a.type] = { total: 0, atrasadas: 0 };
      acc[a.type].total += a.total;
      acc[a.type].atrasadas += a.atrasadas;
      return acc;
    },
    {},
  );
  const chartData = Object.entries(porTipo).map(([type, vals]) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {atividades && (
          <ExportButton
            data={atividades as unknown as Record<string, unknown>[]}
            filename="atividades"
          />
        )}
      </div>
      {isLoading ? (
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Total vs Atrasadas</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
  ...
  const cards = [
    {
      ...
      valor: MetricsService.formatarMoeda(resumo?.previsto_total ?? 0),
      icon: DollarSign,
    },
    {
      titulo: "Total recebido",
      valor: MetricsService.formatarMoeda(resumo?.recebido_total ?? 0),
      icon: DollarSign,
    },
    {
      titulo: "Atrasado",
      valor: MetricsService.formatarMoeda(resumo?.atrasado_total ?? 0),
      icon: DollarSign,
      destaque: (resumo?.atrasado_total ?? 0) > 0,
    },
    {
      titulo: "Recebido este mês",
      valor: MetricsService.formatarMoeda(resumo?.recebido_mes_atual ?? 0),
      icon: DollarSign,
    },
  ];
  const handleExport = () => {
                  ...
                  {comissoes.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="tabular-nums">
                        {MetricsService.formatarMoeda(c.commission_value)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={cn("text-xs font-normal", COMMISSION_STATUS_COLORS[c.status])}
                        >
                      ...
                      <TableCell className="text-muted-foreground">
                        {c.expected_date
                          ? new Date(c.expected_date).toLocaleDateString("pt-BR")
                          : "—"}
                      </TableCell>
                      <TableCell className="text-muted-foreground">
                        {c.received_date
                          ? new Date(c.received_date).toLocaleDateString("pt-BR")
                          : "—"}
                      </TableCell>
                      <TableCell className="text-muted-foreground">
...
function RelatorioDocumentos() {
  ...
  const chartData = (documentos ?? []).map((d) => ({
    name: DOC_LABELS[d.category] ?? d.category,
    total: d.total,
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <KpiCard
          ...
          valor={MetricsService.formatarBytes(totalBytes)}
          icon={FileText}
        />
      ...
      {isLoading ? (
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Documentos por categoria</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} barSize={20}>
...
function RelatorioComunicacao() {
  ...
  const porCanal = (conversas ?? []).reduce<Record<string, number>>((acc, c) => {
    acc[c.channel] = (acc[c.channel] ?? 0) + c.total;
    return acc;
  }, {});
  const chartData = Object.entries(porCanal).map(([channel, total]) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {conversas && (
          <ExportButton
            data={conversas as unknown as Record<string, unknown>[]}
            filename="comunicacao"
          />
        )}
      </div>
      {isLoading ? (
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between border-b px-6 py-4 shrink-0">
        ...
        <Badge variant="outline" className="text-xs gap-1">
          ...
          IA: insights em breve
        </Badge>
      </div>
      <div className="flex-1 overflow-auto p-6">
        <Tabs defaultValue="executivo">
          <TabsList className="mb-6 flex-wrap h-auto gap-1">
            <TabsTrigger value="executivo" className="gap-1.5 text-xs">
