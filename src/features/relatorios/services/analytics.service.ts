...
export const AnalyticsService = {
  ...
  async getTimelineEventos(meses = 6): Promise<TimelineEvento[]> {
    ...
    const { data, error } = await db
      .from("v_analytics_timeline_eventos")
      .select("*")
  ...
  async getFinanceiro(): Promise<{ comissoes: Commission[]; resumo: FinanceiroResumo | null }> {
    ...
    return {
      comissoes:
        comissoesResult.status === "fulfilled"
          ? ((comissoesResult.value.data ?? []) as unknown as Commission[])
          : [],
      resumo:
        resumoResult.status === "fulfilled"
          ? (resumoResult.value.data as unknown as FinanceiroResumo | null)
          : null,
    };
  },
};
