...
export function useAnalyticsKPIs() {
  return useQuery({
    ...
    queryFn: AnalyticsService.getKPIs.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useFunilConversao() {
  return useQuery({
    queryKey: analyticsKeys.funil(),
    queryFn: AnalyticsService.getFunilConversao.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useOrigemLeads() {
  return useQuery({
    queryKey: analyticsKeys.origem(),
    queryFn: AnalyticsService.getOrigemLeads.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useAtividadesAnalytics() {
  return useQuery({
    queryKey: analyticsKeys.atividades(),
    queryFn: AnalyticsService.getAtividades.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useDocumentosAnalytics() {
  return useQuery({
    queryKey: analyticsKeys.documentos(),
    queryFn: AnalyticsService.getDocumentos.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useConversasAnalytics() {
  return useQuery({
    queryKey: analyticsKeys.conversas(),
    queryFn: AnalyticsService.getConversas.bind(AnalyticsService),
    staleTime: 120_000,
  });
}
export function useTimelineEventos(meses = 6) {
  return useQuery({
    queryKey: analyticsKeys.timeline(meses),
    queryFn: () => AnalyticsService.getTimelineEventos(meses),
    staleTime: 120_000,
  });
}
export function useFinanceiroRelatorio() {
  return useQuery({
    queryKey: analyticsKeys.financeiro(),
    queryFn: AnalyticsService.getFinanceiro.bind(AnalyticsService),
    staleTime: 60_000,
  });
}
