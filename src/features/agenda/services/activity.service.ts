...
export async function listarActivities(
  filtros: ActivityFiltros = {},
): Promise<ActivitiesPaginadas> {
  const {
    type,
    status,
    priority,
  ...
  let query = supabase
    .from(TABLE)
    .select("*, client:clients(nome)", { count: "exact" })
    .is("deleted_at", null);
...
export async function criarActivity(payload: ActivityInsert): Promise<Activity> {
  ...
  if (activity.client_id) {
    if (activity.type === "TASK") {
      ...
      TimelineService.followupCriado(
        activity.client_id,
        activity.scheduled_at ?? activity.due_at ?? new Date().toISOString(),
      );
    } else if (activity.type === "VISIT") {
      TimelineService.visitaAgendada(
        activity.client_id,
        activity.metadata?.property_code as string | undefined,
...
export async function concluirActivity(id: string): Promise<Activity> {
  ...
  if (concluida.client_id) {
    if (concluida.type === "TASK")
      TimelineService.tarefaConcluida(concluida.client_id, concluida.id, concluida.title);
    if (concluida.type === "FOLLOWUP")
      TimelineService.followupRealizado(concluida.client_id, concluida.description ?? undefined);
    if (concluida.type === "VISIT")
      TimelineService.visitaRealizada(
        concluida.client_id,
        concluida.metadata?.property_code as string | undefined,
        concluida.description ?? undefined,
      );
    if (concluida.type === "CALL")
      TimelineService.ligacaoRealizada(concluida.client_id, concluida.description ?? undefined);
  }
  return concluida;
}
