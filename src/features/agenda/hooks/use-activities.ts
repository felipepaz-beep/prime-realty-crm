...
export function useActivities(filtros: ActivityFiltros = {}) {
  return useQuery({
    queryKey: activitiesKeys.list(filtros),
    queryFn: () => listarActivities(filtros),
  });
}
...
export function useActivityDetalhe(id: string) {
  return useSuspenseQuery({
    ...
    queryFn: () => buscarActivityPorId(id),
  });
}
...
export function useCriarActivity() {
  ...
  return useMutation({
    mutationFn: (p: ActivityInsert) => criarActivity(p),
...
export function useAtualizarActivity(id: string) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      qc.setQueryData(activitiesKeys.detail(id), data);
    },
  });
}
