...
function KanbanColuna({
  ...
  return (
    <div className="flex flex-col w-64 shrink-0">
      {/* Header da coluna */}
      <div
        className={cn("border border-border rounded-lg border-t-4 bg-muted/30", ETAPA_CORES[etapa])}
      >
        <div className="px-3 py-2.5 flex items-center justify-between">
          <span className="text-xs font-semibold text-foreground">{ETAPA_FUNIL_LABELS[etapa]}</span>
    ...
    >
      <div className="flex gap-4 h-full pb-4">
        {colunas.map(({ etapa, clientes: cols }) => (
          <KanbanColuna key={etapa} etapa={etapa} clientes={cols} isOver={overColuna === etapa} />
        ))}
      </div>

      {/* Card fantasma que segue o cursor */}
      <DragOverlay>{clienteAtivo ? <KanbanCard cliente={clienteAtivo} /> : null}</DragOverlay>
    </DndContext>
  );
}
