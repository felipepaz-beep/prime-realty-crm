...
const EVENT_CONFIG: Record<TimelineEventType, EventConfig> = {
  ...
  venda_concluida: {
    icon: Trophy,
    color: "bg-[oklch(0.62_0.16_155/0.12)] text-[oklch(0.45_0.16_155)]",
    label: "Venda",
  },
  evento_customizado: {
    icon: CalendarDays,
...
function TimelineItem({ event, isLast }: { event: TimelineEvent; isLast: boolean }) {
  ...
  const hora = new Date(event.created_at).toLocaleTimeString("pt-BR", {
    hour: "2-digit",
    minute: "2-digit",
  });
  return (
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center gap-2">
      ...
      <p className="text-sm font-medium">
        {comFiltros ? "Nenhum evento encontrado" : "Nenhum evento registrado"}
      </p>
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <TimelineVazia comFiltros={temFiltros} />
      ) : (
        <div>
          {grupos.map((grupo) => (
            <div key={grupo.label}>
