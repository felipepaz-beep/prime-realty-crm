...
export async function buscarEventosCliente(
  ...
  if (filtros?.busca)
    query = query.or(`title.ilike.%${filtros.busca}%,description.ilike.%${filtros.busca}%`);
...
export const TimelineService = {
  ...
  clienteDeletado(clientId: string, nome: string) {
    return registrarEvento({
      client_id: clientId,
      category: "ciclo_vida",
      event_type: "cliente_deletado",
      title: "Cliente removido",
      description: `${nome} foi removido do CRM.`,
      metadata: {},
    });
  },
  ...
  followupCriado(clientId: string, data: string) {
    return registrarEvento({
      ...
      event_type: "followup_criado",
      title: "Follow-up agendado",
      description: `Próximo contato em ${new Date(data).toLocaleDateString("pt-BR")}.`,
      metadata: { next_date: data },
    });
  },
  ...
  visitaAgendada(clientId: string, codigoImovel?: string, descricao?: string) {
    return registrarEvento({
      ...
      description: descricao,
      metadata: codigoImovel ? { property_code: codigoImovel } : {},
    });
  },
  ...
  financiamentoAprovado(clientId: string, descricao?: string) {
    return registrarEvento({
      client_id: clientId,
      category: "negocio",
      event_type: "financiamento_aprovado",
  ...
  vendaConcluida(clientId: string, valor?: number, formaPagamento?: string) {
    return registrarEvento({
      ...
      description: valor
        ...
        : undefined,
      metadata: { valor, payment: formaPagamento },
    });
  },
  eventoCustomizado(
    clientId: string,
    titulo: string,
    descricao?: string,
    categoria: TimelineCategory = "sistema",
  ) {
    return registrarEvento({
      client_id: clientId,
      category: categoria,
      event_type: "evento_customizado",
      title: titulo,
      description: descricao,
    });
  },
