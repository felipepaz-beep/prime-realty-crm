...
export const CommissionService = {
  async listar(filtros: CommissionFiltros = {}): Promise<CommissionsPaginadas> {
    ...
    let query = supabase.from(TABLE).select("*, client:clients(id, nome)", { count: "exact" });
  ...
  async criar(payload: Omit<CommissionInsert, "owner_id">): Promise<Commission> {
    ...
    if (commission.client_id) {
      TimelineService.vendaConcluida(
        commission.client_id,
        commission.gross_value,
        commission.payment_method ?? undefined,
      );
    }
    return commission;
  },
  ...
  async resumo(): Promise<FinanceiroResumo | null> {
    const { data, error } = await supabase.from("v_financeiro_resumo").select("*").maybeSingle();
    if (error) throw error;
