            ...
            <div key={c.id} className="flex items-center gap-3 rounded-lg border p-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={cn("text-[10px] h-4", COMMISSION_STATUS_COLORS[c.status])}
                  >
                ...
                <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                  ...
                  <span>·</span>
                  <span>{Number(c.commission_percentage)}%</span>
                  {c.property_code && (
                    <>
                      <span>·</span>
                      <span>Cód: {c.property_code}</span>
                    </>
                  )}
                  {c.expected_date && (
                    <>
                      <span>·</span>
                      <span>
                        Prev: {new Date(c.expected_date + "T12:00").toLocaleDateString("pt-BR")}
                      </span>
                    </>
                  )}
                </div>
