...
export async function listarConversas(
  filtros: ConversationFiltros = {},
): Promise<ConversationPaginadas> {
  const { channel, status, busca, pagina = 1, porPagina = 20 } = filtros;
  let query = supabase
    .from(CONV_TABLE)
    .select(
      "*, client:clients(nome,telefone,whatsapp,email,etapa_funil,prioridade,proximo_followup,ultimo_contato,tags)",
      { count: "exact" },
    )
    .is("deleted_at", null)
...
export async function iniciarConversa(payload: ConversationInsert): Promise<Conversation> {
  ...
  TimelineService.whatsappEnviado(
    payload.client_id,
    `Conversa iniciada via ${payload.channel ?? "whatsapp"}`,
  );
  return conversa;
}
...
export async function enviarMensagem(
  payload: MessageInsert & { client_id: string },
): Promise<Message> {
  const { client_id, ...msgPayload } = payload;
  const { data, error } = await supabase
    .from(MSG_TABLE)
    .insert({ ...msgPayload, direction: "outgoing" } as never)
    .select("*")
    .single();
  if (error) throw error;
  ...
  try {
    ...
    if (contactId && (await provider.isConnected())) {
      const externalId =
        msg.type === "text" && msg.content
          ...
          : msg.attachment
            ? await provider.sendMedia(contactId, msg.attachment)
            : "";
      if (externalId) {
        await supabase
          .from(MSG_TABLE)
          .update({
            metadata: { ...msg.metadata, provider_msg_id: externalId },
            status: "sent",
          } as never)
          .eq("id", msg.id);
      }
    ...
    console.warn("[CommunicationService] Falha no provider:", e);
  }
  if (msg.type === "text") TimelineService.whatsappEnviado(client_id, msg.content ?? undefined);
  else if (msg.attachment)
    TimelineService.whatsappEnviado(client_id, `Arquivo enviado: ${msg.attachment.name}`);
  return msg;
}
...
export async function receberMensagem(
  ...
  const { data, error } = await supabase
    .from(MSG_TABLE)
    .insert({ ...msgPayload, direction: "incoming" } as never)
    .select("*")
    .single();
  if (error) throw error;
  const msg = data as Message;
  if (msg.type === "text") TimelineService.whatsappRecebido(client_id, msg.content ?? undefined);
    ...
    TimelineService.whatsappRecebido(client_id, `Arquivo recebido: ${msg.attachment.name}`);
  return msg;
}
