...
function ResponseCard({
  ...
  return (
    <div className="rounded-lg border bg-muted/30 overflow-hidden">
      ...
      <div className="p-3">
        <p className="text-sm whitespace-pre-wrap leading-relaxed">{response.content}</p>
      </div>
      {onSalvar && (
        <div className="border-t px-3 py-2 flex justify-end">
          <Button
            ...
            onClick={() => onSalvar(response.content)}
          >
            Salvar como nota
          </Button>
        </div>
...
function QuickActionButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex flex-col items-center gap-1.5 p-2 rounded-lg border text-center transition-colors hover:bg-muted/60 hover:border-primary/30",
...
export function AIDrawer({ clientId, clienteNome, children, onSalvar }: AIDrawerProps) {
  ...
  const handleExecutar = useCallback(
    async (action: AIAction, cat: AICategory, userPrompt: string) => {
      if (!userPrompt.trim()) return;
      try {
        const response = await executar.mutateAsync({
          action,
          category: cat,
          userPrompt,
          clientId,
        ...
        toast.error("Erro ao executar IA. Tente novamente.");
      }
    },
    [executar, clientId],
  );
  ...
  const handleQuickAction = useCallback(
    (action: AIAction, cat: AICategory) => {
      handleExecutar(
        action,
        cat,
        clienteNome ? `Cliente: ${clienteNome}` : "Contexto geral do CRM",
      );
    },
        ...
        <ScrollArea className="flex-1">
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                placeholder={`O que você quer criar?${clienteNome ? ` (para ${clienteNome})` : ""}`}
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && e.ctrlKey) {
                    e.preventDefault();
                    handleExecutar("custom", categoria, prompt);
                  }
                }}
              />
              <Button
                ...
                size="sm"
                onClick={() => handleExecutar("custom", categoria, prompt)}
                disabled={!prompt.trim() || executar.isPending}
              >
                {executar.isPending ? (
                  <>
            ...
            {historico.length > 0 ? (
              <div className="space-y-3">
                <p className="text-xs font-medium text-muted-foreground">
                  Respostas recentes ({historico.length})
                </p>
                {historico.map((r, i) => (
                  <ResponseCard key={i} response={r} onSalvar={onSalvar} />
                ))}
              </div>
            ) : (
              !executar.isPending && (
                <div className="flex flex-col items-center justify-center py-8 gap-2 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Selecione uma ação rápida ou escreva um prompt para começar.
                  </p>
                </div>
              )
            )}
