...
import { useMensagensCliente, useAnalisarConversa, useChatCopiloto } from "../hooks/use-copiloto";
...
function parseWhatsAppChat(text: string, nomeProprietario?: string): MensagemConversa[] {
  ...
  for (const linha of linhas) {
    ...
    if (match) {
      ...
      const isProprietario = nomeProprietario
...
async function extrairTextoDoChatZip(file: File): Promise<string> {
  ...
  const chatEntry = Object.values(zip.files).find((f) => f.name.endsWith(".txt") && !f.dir);
...
function ScoreRing({ score }: { score: number }) {
  const cor = score >= 70 ? "text-emerald-500" : score >= 40 ? "text-amber-500" : "text-rose-500";
        ...
        <div className="space-y-3 py-2">
          {historico.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-xs">
                Pergunte qualquer coisa sobre {clienteNome}.<br />O copiloto usa o histórico real de
                conversa.
              </p>
            </div>
          )}
    ...
    <div className="space-y-5 max-w-md mx-auto py-4">
      ...
      <div className="space-y-2">
        ...
        {!modoTexto ? (
          ...
          <Textarea
            placeholder={
              "10/06/2024, 14:32 - Carlos: Olá, vi o imóvel...\n10/06/2024, 14:33 - Felipe: Oi Carlos..."
            }
            value={textoColado}
...
function PreviewConversaCRM({
  ...
  return (
    <PreviewConversa clienteNome={clienteNome} mensagens={(msgs ?? []) as MensagemConversa[]} />
  );
}
