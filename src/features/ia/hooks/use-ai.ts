...
export function useAIExecute() {
  return useMutation({
    mutationFn: (params: {
      ...
      temperature?: number;
      maxTokens?: number;
    }) => AIService.execute(params),
  });
}

export function usePrompts(category?: AICategory) {
  return useQuery({
    queryKey: aiKeys.prompts(category),
    queryFn: () => PromptLibrary.listar(category),
    staleTime: 60_000,
  });
}
...
export function useCriarPrompt() {
  ...
  return useMutation({
    mutationFn: (p: Partial<Record<string, unknown>>) => PromptLibrary.criar(p),
...
export function useAtualizarPrompt() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Partial<Record<string, unknown>> }) =>
      PromptLibrary.atualizar(id, payload),
    onSuccess: () => qc.invalidateQueries({ queryKey: aiKeys.all }),
  });
}
...
export function useToggleFavoritoPrompt() {
  ...
  return useMutation({
    mutationFn: ({ id, favorite }: { id: string; favorite: boolean }) =>
      PromptLibrary.toggleFavorito(id, favorite),
    onSuccess: () => qc.invalidateQueries({ queryKey: aiKeys.all }),
  });
}
...
export function useRemoverPrompt() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => PromptLibrary.remover(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: aiKeys.all }),
  });
}
...
export function useAIRecentLogs() {
  return useQuery({
    queryKey: aiKeys.logs(),
    queryFn: () => UsageService.getRecentLogs(20),
    staleTime: 30_000,
  });
}
