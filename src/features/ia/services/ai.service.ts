...
class StubAIProvider implements AIProvider {
  ...
  models: string[];
  defaultModel: string;
  constructor(name: AIProviderName, models: string[], defaultModel: string) {
    this.name = name;
    this.models = models;
  ...
  async route(request: AIRequest): Promise<AIResponse> {
    ...
    for (const providerName of order) {
      ...
      try {
        ...
        if (this.config.cacheEnabled)
          CACHE.set(cacheKey(request), { response, expiresAt: Date.now() + CACHE_TTL });
        return response;
      } catch {
        if (!fallbackFrom) fallbackFrom = providerName;
      }
    }
    const stubResponse = await AI_PROVIDERS.stub.complete(request);
    stubResponse.fromFallback = true;
    stubResponse.fallbackFrom = fallbackFrom;
    return stubResponse;
  }

  updateConfig(partial: Partial<AIRouterConfig>): void {
...
export const ContextBuilder = {
  async buildForClient(clientId: string): Promise<AIContext> {
    const [clientResult, timelineResult] = await Promise.allSettled([
      supabase
        .from("clients")
        .select("nome,telefone,email,etapa_funil,temperatura,score,observacoes,tags")
        .eq("id", clientId)
        .single(),
      supabase
    ...
    if (clientResult.status === "fulfilled" && clientResult.value.data) {
      ...
      context.client = {
        ...
        etapa_funil: String(c.etapa_funil ?? ""),
        temperatura: String(c.temperatura ?? ""),
    ...
    if (timelineResult.status === "fulfilled" && timelineResult.value.data) {
      context.recentTimeline = (timelineResult.value.data as Array<Record<string, unknown>>).map(
        (t) => ({
...
export const UsageService = {
    ...
    try {
      await supabase
        ...
        .insert({
          ...
          prompt_tokens: params.response.promptTokens,
          output_tokens: params.response.outputTokens,
          total_tokens: params.response.totalTokens,
          cost_usd: params.response.costUsd,
          duration_ms: params.response.durationMs,
          status: params.response.fromFallback ? "fallback" : (params.status ?? "success"),
          error_message: params.errorMessage ?? null,
          fallback_from: params.response.fallbackFrom ?? null,
  ...
  async getSummary(days = 30): Promise<AIUsageSummary> {
    ...
    const { data } = await supabase
      .from("ai_usage_logs")
      .select("status,provider,category,total_tokens,cost_usd,duration_ms")
    ...
    const summary: AIUsageSummary = {
      ...
      fallback_count: logs.filter((l) => l.status === "fallback").length,
      total_tokens: logs.reduce((s, l) => s + (Number(l.total_tokens) || 0), 0),
      total_cost_usd: logs.reduce((s, l) => s + (Number(l.cost_usd) || 0), 0),
      avg_duration_ms: logs.length
        ? logs.reduce((s, l) => s + (Number(l.duration_ms) || 0), 0) / logs.length
        : 0,
      by_provider: {},
      by_category: {},
    };
    for (const log of logs) {
      ...
      const c = String(log.category);
      summary.by_provider[p] = (summary.by_provider[p] ?? 0) + 1;
...
export const AIService = {
  async execute(params: {
    ...
    try {
      ...
      response = await AI_PROVIDERS.stub.complete({ messages, action, category });
      response.fromFallback = true;
    }
    UsageService.log({
      action,
...
export const PromptLibrary = {
  ...
  async toggleFavorito(id: string, favorite: boolean) {
    await supabase
      .from("ai_prompts")
