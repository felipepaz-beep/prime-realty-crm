...
export async function fetchPendingChats(): Promise<PendingChat[]> {
  ...
  const chatsParaAnalisar = chats.filter((c: any) => c.id && c.id !== "null").slice(0, 50);
  ...
  for (const chat of chatsParaAnalisar) {
    ...
    const msgRes = await fetch(`${baseUrl}/chat/findMessages/${instance}`, {
      method: "POST",
      headers: { apikey: apiKey, "Content-Type": "application/json" },
    ...
    const messages: ChatMessage[] = (msgData.messages?.records ?? [])
      .map((m: any) => ({
...
Responda APENAS com o texto da mensagem.`,
  ...
  return (
    data.content?.[0]?.text ??
    `Olá ${clientName}! Tudo bem? Estou à disposição caso queira retomar a busca pelo imóvel ideal. 😊`
  );
}
...
export async function runWhatsAppAgent(): Promise<AgentRunResult> {
  ...
  try {
    ...
    for (const client of candidates.slice(0, 10)) {
      // máximo 10 por vez
