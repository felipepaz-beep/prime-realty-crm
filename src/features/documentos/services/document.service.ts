...
export function validarArquivo(file: File): ValidationResult {
  ...
  if (file.size > MAX_FILE_SIZE)
    return {
      valid: false,
      error: `O arquivo excede o limite de 50MB (${formatarTamanho(file.size)}).`,
    };
...
export async function listarDocumentos(
  clientId: string,
  filtros: DocumentFiltros = {},
): Promise<DocumentsPaginados> {
  const {
    category,
    busca,
    favorite,
  ...
  if (busca)
    query = query.or(
      `file_name.ilike.%${busca}%,original_name.ilike.%${busca}%,description.ilike.%${busca}%`,
    );
...
export async function uploadDocumento(options: UploadOptions): Promise<ClientDocument> {
  ...
  const { data, error: dbError } = await supabase
    .from(TABLE)
    .insert({ ...payload, owner_id: ownerId, uploaded_by: ownerId } as never)
    .select("*")
    .single();
  if (dbError) {
...
export async function atualizarDocumento(
  ...
  if (payload.file_name)
    TimelineService.documentoAnexado(doc.client_id, doc.file_name, doc.storage_path);
  return doc;
}
...
export async function restaurarDocumento(id: string): Promise<ClientDocument> {
  ...
  if (error) throw error;
  const doc = data as ClientDocument;
  TimelineService.documentoAnexado(
    doc.client_id,
    `${doc.file_name} (restaurado)`,
    doc.storage_path,
  );
  return doc;
}
