...
interface DocCardProps {
  ...
  onFavoritar: (doc: ClientDocument) => void;
  onEditar: (doc: ClientDocument) => void;
  onRemover: (doc: ClientDocument) => void;
...
function DocCard({
    ...
    return (
      <div className="flex items-center gap-3 py-2.5 px-3 rounded-lg hover:bg-muted/40 group border border-transparent hover:border-border transition-all">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {canPreview && (
            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onPreview(doc)}>
              <FileText className="h-3.5 w-3.5" />
            </Button>
          )}
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onDownload(doc)}>
  ...
  return (
    <div className="group border rounded-lg overflow-hidden bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
      <div
        className="h-28 bg-muted flex items-center justify-center cursor-pointer relative"
        onClick={() => canPreview && onPreview(doc)}
      >
        {isImage ? (
          <PreviewImage storagePath={doc.storage_path} alt={doc.file_name} />
        ) : (
          <FileIcon mimeType={doc.mime_type} className="h-10 w-10 text-muted-foreground/40" />
        )}
        {doc.favorite && (
      ...
      <div className="p-2.5">
        ...
        <div className="flex items-center gap-1 mt-2">
          ...
          <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => onDownload(doc)}>
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground mt-1">
        PDF, imagens, Word, Excel · Máx. 50MB por arquivo
      </p>
      <input
        ref={inputRef}
        type="file"
...
function EditModal({
  ...
  onSave: (
    id: string,
    payload: { file_name: string; category: DocumentCategory; description: string },
  ) => void;
  isLoading: boolean;
}) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => onSave(doc.id, { file_name: nome, category: cat, description: desc })}
...
export function ClienteDocumentos({ clienteId }: { clienteId: string }) {
  ...
  const handleFiles = useCallback(
    async (files: File[]) => {
      ...
      const novos: UploadProgress[] = files.map((file) => ({
        file,
        progress: 0,
        status: "pending",
      }));
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!validation.valid) {
          setUploads((prev) =>
            prev.map((u, idx) =>
              idx === base + i ? { ...u, status: "error", error: validation.error } : u,
            ),
          );
        ...
        setUploads((prev) =>
          prev.map((u, idx) => (idx === base + i ? { ...u, status: "uploading" } : u)),
        );
        try {
          await uploadDocumento({
            ...
            onProgress: (p) =>
              setUploads((prev) =>
                prev.map((u, idx) => (idx === base + i ? { ...u, progress: p } : u)),
              ),
          });
          setUploads((prev) =>
            prev.map((u, idx) => (idx === base + i ? { ...u, status: "done", progress: 100 } : u)),
          );
          toast.success(`${file.name} enviado!`);
        } catch (err) {
          const msg = err instanceof Error ? err.message : "Erro no upload";
          setUploads((prev) =>
            prev.map((u, idx) => (idx === base + i ? { ...u, status: "error", error: msg } : u)),
          );
  ...
  const handleRemover = useCallback(
    async (doc: ClientDocument) => {
      ...
      await remover.mutateAsync(doc.id);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(v) => setCategoriaFiltro(v === "all" ? "" : (v as DocumentCategory))}
        >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {busca ? "Tente outro termo." : "Envie o primeiro documento acima."}
            </p>
          </div>
        </div>
      ) : view === "grid" ? (
      ...
      {documentos.length > 0 && (
        <p className="text-xs text-muted-foreground text-center">
          {documentos.length} documento{documentos.length !== 1 ? "s" : ""} · {data?.total ?? 0}{" "}
          total
        </p>
      )}
