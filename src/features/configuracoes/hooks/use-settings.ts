...
export function useSettings(category: SettingsCategory) {
  return useQuery({
    ...
    queryFn: () => SettingsService.getCategory(category),
    staleTime: 300_000,
  });
}
...
export function useSalvarSettings(category: SettingsCategory) {
  ...
  return useMutation({
    ...
    onMutate: async (values) => {
      ...
      qc.setQueryData(settingsKeys.category(category), (old: Record<string, unknown>) => ({
        ...old,
        ...values,
      }));
      return { prev };
    ...
    onError: (_, __, ctx) => {
      qc.setQueryData(settingsKeys.category(category), ctx?.prev);
    },
    onSettled: () => {
...
export function useIntegrations() {
  return useQuery({
    ...
    queryFn: IntegrationService.listar.bind(IntegrationService),
    staleTime: 60_000,
  });
}
