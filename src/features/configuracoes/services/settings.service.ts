...
export const SettingsService = {
  ...
  async get<T>(category: SettingsCategory, key: string, defaultValue: T): Promise<T> {
    const { data } = await db
      ...
      .eq("key", key)
      .maybeSingle();
    return (data as any)?.value !== undefined && (data as any)?.value !== null
      ? ((data as any).value as T)
      : defaultValue;
  },
  async set(category: SettingsCategory, key: string, value: unknown): Promise<void> {
    ...
    const { error } = await db
      .from("user_settings")
      .upsert({ owner_id: ownerId, category, key, value }, { onConflict: "owner_id,category,key" });
    if (error) throw error;
  },
  async setMany(category: SettingsCategory, values: Record<string, unknown>): Promise<void> {
    ...
    const { error } = await db
      .from("user_settings")
      .upsert(rows, { onConflict: "owner_id,category,key" });
    if (error) throw error;
  },
...
export const IntegrationService = {
  ...
  async salvar(
    ...
    const { data, error } = await db
      .from("integrations")
      .upsert(
        { owner_id: ownerId, provider, status, configuration },
  ...
  async desconectar(provider: IntegrationProvider): Promise<void> {
    await this.salvar(provider, "disconnected", {});
  },
