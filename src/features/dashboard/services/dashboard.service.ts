...
export async function buscarClientesPrioritarios(): Promise<ClientePrioritario[]> {
  ...
  if (error) throw error;
  return (data ?? []) as ClientePrioritario[];
}
...
export async function buscarIndicadores(): Promise<DashboardIndicadores> {
  ...
  if (error) throw error;
  return data as DashboardIndicadores;
}
export async function buscarAtividadesDoDia(): Promise<AtividadeDashboard[]> {
  ...
  const { data, error } = await supabase
    .from("activities")
    .select(
      "id,type,title,description,status,priority,scheduled_at,due_at,client_id,client:clients(nome)",
    )
    .is("deleted_at", null)
    ...
    .limit(20);
  if (error) throw error;
  return (data ?? []) as AtividadeDashboard[];
}
...
export async function pesquisarGlobal(termo: string): Promise<ResultadoPesquisa[]> {
  ...
  const { data, error } = await supabase
    ...
    .is("deleted_at", null)
    .or(
      `nome.ilike.%${termo}%,telefone.ilike.%${termo}%,whatsapp.ilike.%${termo}%,email.ilike.%${termo}%,codigo_imovel.ilike.%${termo}%`,
    )
    .limit(8);
  if (error) throw error;
  return (data ?? []) as ResultadoPesquisa[];
}
